...
const startFyberActivity = (userid, appkey, appid) => {
  ...
  const handlePlayButtonPress = (offer) => {
    if (offer.data.includes("http")) {
      ...
      console.log(placementName[0].value +'###'+ placementName[1].value +'###'+ apiToken);
	  if(offer.title == 'Tapjoy'){
      startMyKotlinActivity(apiToken, placementName[0].value, placementName[1].value);
	  }else if (offer.title == 'Fyber'){}
     startFyberActivity(apiToken, placementName[1].value,placementName[0].value)
    }
  };
